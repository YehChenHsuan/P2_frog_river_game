...
const VOCAB_MAP = new Map(P2_VOCABULARY.map(v => [v.id, v]));
...
class QuestionGenerator {
  ...
  static generateListenQuestion() {
    const target = P2_VOCABULARY[Math.floor(Math.random() * P2_VOCABULARY.length)];
    const others = P2_VOCABULARY.filter(v => v.id !== target.id);
  ...
  static generatePhonicsQuestion() {
    ...
    const target = validTargets.length > 0 ? validTargets[Math.floor(Math.random() * validTargets.length)] : P2_VOCABULARY[0];
    
    // 干擾項
    const distractors = P2_VOCABULARY.filter(v => !targetWordIds.includes(v.id));
  ...
  static generateActionQuestion() {
    ...
    const target = VOCAB_MAP.get(qData.correct) || {
      ...
      word: qData.correct,
      zh: qData.correct,
      image: `P2_flashcards_images/P2_${qData.correct}.webp`,
      audioEn: `P2_flashcards_audios/P2_${qData.correct}.mp3`,
      audioZh: `P2_flashcards_audios/P2_${qData.correct}_zh.mp3`
    };
    ...
    const optionItems = qData.options.map(id => VOCAB_MAP.get(id) || {
      ...
      word: id,
      zh: id,
      image: `P2_flashcards_images/P2_${id}.webp`,
      audioEn: `P2_flashcards_audios/P2_${id}.mp3`,
      audioZh: `P2_flashcards_audios/P2_${id}_zh.mp3`
    });
